...
- Export renders via ffmpeg to `Exports/`, normalized crop/scale/fps. Consecutive segments crossfade into each other (0.4s, both video and audio) instead of hard-cutting -- applies between whichever two segments are adjacent in the current sequence order. A source clip with no audio track (silent b-roll) gets silence substituted for that segment rather than crashing the export
- Every export is hard-capped at the 30s/60s target, frame-accurate -- even if the Hook/Gameplay/Endcard segments add up to more than the target (the timeline's "+Xs" mismatch warning), the rendered file is trimmed to the target duration exactly, never a frame over. Segments that already fit are untouched
...
- "File naming" bar sets the export filename structure: `{Game}_{Dimension}_{Date}_{Language}_{HookName}_{GPName}_{Duration}s.mp4` (e.g. `GardenGetaway_9x16_2026.09.14_EN_HookUGCYoungLady_GPFullBoard_30s.mp4`). "Game" is a free-text field (remembered across reloads), "Language" is EN/ES/IT/TR/DE, and the rest fill in automatically from your current selections -- a live preview shows the exact filename before you export. HookName/GPName are capped to the first 3 words of the source filename (so a long Higgsfield-generated hook name like `Generated_add_idle_animation_to_the_reference_...` becomes just `Generatedaddidle`), and never include anything from the Endcard or CTA. Re-exporting the same recipe on the same day overwrites the existing file rather than piling up random suffixes
...
Either way, captioning happens in place -- the export is replaced with its captioned version rather than keeping both, so checking "Caption this" leaves exactly one file behind. Requires speech in the audio; silent gameplay/endcard segments just won't have cues (the plain export is left as-is if no speech is detected).
